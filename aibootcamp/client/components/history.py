...
def render_history_ui():
    ...
    # 상담내역 이력 로드
    adviceitem_history = fetch_adviceitem_history()

    if not adviceitem_history:
        st.info("저장된 상담내역 이력이 없습니다.")
    else:
        render_history_list(adviceitem_history)
        ...
        with st.container(border=True):
            ...
            with col2:
                if st.button("보기", key=f"view_{id}", use_container_width=True):
                    topic, messages, docs = fetch_adviceitem_by_id(id)
                    if topic and messages:
                        st.session_state.viewing_history = True
                        st.session_state.messages = messages
                        st.session_state.loaded_topic = topic
                        st.session_state.loaded_adviceitem_id = id
